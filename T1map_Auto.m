...
mkdir T1maps
[index_1,index_2] = xlsread('19.abb.14_120720.xlsx',2,'A2:M59');
[index_3,index_4] = xlsread('Inversion_Efficiencies.xlsx',1,'A2:M58');
...
T1_list = dir(['/Users/zijiandong/Desktop/research/Sedentary/','*T1maps.nii.gz']);
for a = 1:size(index_2,1)
    if contains(index_2,'190715')
        continue
    end
    for b = 1:length(T1_list)
        ...
        if strcmp(name_1,name_2)
            T1 = niftiread(['/Users/zijiandong/Desktop/research/Sedentary/',T1_list(b).name]);
%             if size(T1,3) ~= 30
%                 T1_inL = T1;
...
% flip
            ...
            m = {};
            index = [];
            for num = 1:size(M_z,1)
                t = sort(M_z(num,:));
                m{num} =find(M_z(num,:)<=t(3),3);
                index(num) = max(m{num});
            end
%             [min,index]=min(M_z,[],2);
            for k = 1:100*100*30
...
%     pause
% end
                fun=@(x)sseval(x, TI(1:15), double(M_z(i,1:15)).',IE); 
                x0=[0 0]; 
%                 options=optimset('MaxFunEvals',10000); 
                curveFit=fmincon(fun, x0,[],[],[],[], [0 0],[Inf,Inf]); 
...
%             T1vals = imfilter(T1vals,gf);
            ...
            nii = load_untouch_nii(['/Users/zijiandong/Desktop/research/Sedentary/',T1_list(b).name]);
            nii.img = T1vals;
            ...
            save_untouch_nii(nii,[extractBefore(index_2{a,1},':'),'.nii.gz'])
            display(index_2{a,1})
...
gradient = ((2.*IE.*M0)./T1).*exp(-TIdata./T1);
sse=sum((MZdata-LMmod).^2)+0.005*sum(sum(1./(gradient+0.0001)));
end
